...
/*
 * HymoFS - proc, mountinfo, maps, and read-path hooks layered over syscall flow.
 *
...
int hymofs_proc_hooks_init(bool skip_getfd, bool no_tracepoint, bool skip_extra_kprobes)
{
	...
	if (!skip_extra_kprobes && !hymofs_tracepoint_path_registered()) {
